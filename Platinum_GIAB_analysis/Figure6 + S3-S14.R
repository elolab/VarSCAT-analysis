...
make_plot <- function(results_frame,output_name,num_list,per_list)
{
  ...
  for (i in 1:8)
  {
    ...
    index_2 <- i+8
    index_3 <- i+16
    if (output_name=="Variants not in STR regions")
    {
...
make_plot(TR_long,"Variants in STR regions",TR_num_list,TR_per_list)
make_plot(non_TR_long,"Variants not in STR regions",non_TR_num_list,non_TR_per_list)
